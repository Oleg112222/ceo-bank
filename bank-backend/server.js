...
// Ця функція перевірить, чи є адмін, і якщо немає - створить його з паролем, який точно буде працювати.
async function createInitialAdminIfNeeded() {
    ...
    try {
        ...
        if (res.rows.length === 0) {
            console.log('Адміністратор "admin" не знайдений. Створюємо нового...');
            const password = 'admin'; // Пароль для входу
...
pool.connect((err, client, release) => {
  ...
  release();
  // Запускаємо функцію створення адміна після успішного підключення
...
/*
 * =================================================================
 * АУТЕНТИФІКАЦІЯ (З ВАШОЮ ЛОГІКОЮ)
 * =================================================================
 */
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    // Ваша логіка для обходу перевірки пароля для адміна
    if (username === "admin" && password === "admin") {
       isPasswordCorrect = true;
    } else {
        // Стандартна перевірка пароля для інших користувачів
